...
![CKAN MTE](https://img.shields.io/badge/CKAN%20MTE-2.0.0%20%20développement%20-success.svg)
...
- [1- Installation de l'environnement de développement docker pour CKAN](#installation-de-l-environnement-de-developpement-docker)
    - [1.1- Moissonner des données](#moissonner-des-données)
      - [1.1.1- Ajout une source de moissonnage](#ajouter-une-source-de-moissonnage)
...
A noter que vous pourriez rencontrer diffrents problèmes lors du lancement du docker-compose, dans ces cas, vous pourriez effécter l'une/ces action(s): 
```bash
...
### 3.1- Test moissonnge (parse_dataset)
``` 
...
- Le script de moissonnage est localisé dans le module <i>ckanext-ecospheres\ckanext\ecospheres\dcat\profiles\dataset</i> et les tests associés dans le fichier <i>src\ckanext-ecospheres\ckanext\ecospheres\tests\test_parser_french_profile.py</i>
   - le fichier **<i>parse_dataset.py</i>** contient la fonction pricipale de parsing **parse_dataset**
   - le fichier <i>**_function.py**</i> contient des les fonctions unitaires de parsing 

- Toutes les metadonnées spécifiées dans le fichier scheming sont parsées et testé 
- Les métadonnées relatives aux thèmes et sous-thèmes ne sont pas testé à date (tests à faire)
...
- l'alimentation du champs territoire se fait ( à date) de manière suivante:
    - Une fois que la fonction **<i>parse_dataset.py</i>** ait renvoyé son resultat, on verifie si le champs **spatial** est présent.
    - On se fait rien si le champ **spatial** est renseigné
    - Dans le cas contraire, on récupere les informations relatives à l'organisation liée à la source de  moissonnage et les informations sur la localisation et coordonnées et calculer le champ **spatial**
    - A date, le champ **spatial** n'est pas colculer ( A faire) 
- Le mapping des formats des ressorces se fait (à date ) au moment du moissonnage. Cette méthode ne me semble pas pertinente vu qu'il se pourrait qu'il ait de nouveaux formats à ajouter et le fait qu'on va gérer ce <i>mapping</i> comme un vocabulaire externalisé. Donc le mapping des foramts de ressources se fera au moment de la restitution de l'ecran à l'utilisateur

- Les dévelopements réalisés à date sont en attente de validation. 
...
Le calcul interviendra après chaque phase de moissonage, on attendra que tous les moissonnage aient fini pour lancer le job de calcul, le resultat sera stocké en base de donnnées et exposé à travers un API ou fonction ( à voir ce qui plus pertinent) afin d'être consommé par le front.   
<br>
...
|   **Filter par date de mise à jour**      |Non |        Fait. Example de requêtes: [1]   | 
...
- L'ingestion et le stockage en base de données des  vocabulaires Territoires et thèmes sont faits, et utilisé notamment pour le mapping des themes et des territoires lors du moissonnage. Cependant, après un atelier de travail avec Lislie le mercredi 3 août et vu le nombre de vocabulaires à traiter, on va etre amené à modifier et faire evoluer cette fonctionnalité pour le rendre plus générique.
- Par consequent, Leslie s'occupera du parsing des vocabulaires, et de mon côté, je m'occuperai de la partie mise en base.
