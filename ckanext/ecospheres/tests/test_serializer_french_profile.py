...
class TestEuroDCATAPProfileSerializeDataset(BaseSerializeTest):
    ...
    def test_graph_from_dataset(self):
        ...
        """ subject """
        # assert self._triple(g, dataset_ref,  DCT.subject, dataset['subject'])
        ...
        assert self._triple(g, temp_exts, DCT.startDate,  parse_date(temporal_dict["start_date"]).isoformat(), XSD.dateTime)
        assert self._triple(g, temp_exts, DCT.endDate,  parse_date(temporal_dict["end_date"]).isoformat(), XSD.dateTime)
        ...
        # NAME {*}
        # > foaf:name
        for x in  self._triples(g, publisher_node, VCARD.name, None):
        ...
        assert self._triple(g, affiliation_node, FOAF.name, publisher_dict["affiliation"])
        ...
        assert affiliation_node
        assert self._triple(g, affiliation_node, RDF.type, ORG.Organization)
        assert self._triple(g, affiliation_node, FOAF.name, creator_dict["affiliation"])
        ...
        # NAME {*}
        # > foaf:name
        for x in  self._triples(g, rightsHolder_node, VCARD.name, None):
        ...
        assert affiliation_node
        assert self._triple(g, affiliation_node, RDF.type, ORG.Organization)
        assert self._triple(g, affiliation_node, FOAF.name, rightsHolder_dict["affiliation"])
        ...
        for qualified_attribution in qualified_attribution_node:
            ...
            # HAD_ROLE [{}]
            # > dcat:hadRole
            assert self._triple(g, qualified_attribution, DCAT.hadRole, agent_qualified_attribution_dict[0]['had_role'])
            ...
            for agent in agent_qualified_attribution_node:
                ...
                #foaf:givenName
                assert self._triple(g, agent, FOAF.givenName, agent_qualified_attribution_dict[0]['agent'][0]["name"])
        ...
        for primary in is_primary_topic_of_node:
            ...
            for x in  self._triples(g, contact_point_node, VCARD.name, None):
        ...
        for a in crs_nodes:
            ...
            if str(a) == dataset["conforms_to"][0]["uri"]:
                ...
                assert self._triple(g, a,  DCT.title, dataset["conforms_to"][0]["title"])
            # else:
            #     """------------------------------------------ crs ------------------------------------------"""
